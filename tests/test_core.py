...
class TestLazyResult(unittest.TestCase):
    def setUp(self):
        from wrapconfig.core import WrapConfig, LazyResult

        # Creating a dummy WrapConfig instance
        class MockWrapConfig(WrapConfig):
            def load(self):
                pass  # No-op implementation
        ...
        self.manager = MockWrapConfig()
        self.key = "dummy_key"
        self.lazy_result = LazyResult(self.manager, self.key)

    def test_initialization(self):
        from wrapconfig.core import LazyResult

        # Ensure LazyResult is stored in the parent config
        self.assertIsInstance(self.manager._data[self.key], LazyResult)

    def test_set_item(self):
        # Set an item on the LazyResult instance
        sub_key = "sub_key"
        value = "test_value"
        self.lazy_result[sub_key] = value

        # Ensure it updates the parent config correctly
        self.assertEqual(self.manager._data[self.key][sub_key], value)

    def test_get_item(self):
        # Request an item from the LazyResult instance
        from wrapconfig.core import LazyResult

        sub_key = "another_sub_key"
        result = self.lazy_result[sub_key]

        # Ensure a new LazyResult is created for the sub_key in the parent config
        self.assertIsInstance(result, LazyResult)
        self.assertIsInstance(self.manager._data[self.key][sub_key], LazyResult)

    def test_repr(self):
        # Ensure the representation string is as expected
        expected_repr = f"<LazyResult parent={self.manager} key={self.key}>"
        self.assertEqual(repr(self.lazy_result), expected_repr)

    def test_solve_lazy_result_subconfig(self):
        # Test that a LazyResult can be solved
        from wrapconfig.core import LazyResult, SubConfig

        lr = self.manager[self.key]
        self.assertIsInstance(lr, LazyResult)
        lr["sub_key"] = "value"
        self.assertEqual(self.manager._data[self.key]["sub_key"], "value")
        self.assertEqual(self.manager.data, {self.key: {"sub_key": "value"}})
        lr = self.manager[self.key]
        self.assertNotIsInstance(lr, LazyResult)
        self.assertIsInstance(lr, SubConfig)
